...
def infer_default():
    I_cat_pred_dir = os.path.join(result_dir, 'I_cat_pred')
    util.ensure_dir(I_cat_pred_dir)
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(tqdm(data_loader, ascii=True)):
            ...
            # get network output
            # (N, 4*C, H, W) GPU tensor
            I_cat_pred = model(L_cat)

            # save data
            I_cat_pred_numpy = np.transpose(I_cat_pred.squeeze().cpu().numpy(), (1, 2, 0))
            np.save(os.path.join(I_cat_pred_dir, name + '.npy'), I_cat_pred_numpy)
